import { size, filter, map } from "lodash-es"
...
export default async function spider(config) {
  ...
  print(`Add ${length} new questions`)
  ...
  const result = await Promise.all(
    map(questions, (question) => {
      ...
      if (exists && exists.titleSlug === titleSlug) {
        const unAcceptedLanguages = filter(
          languages,
          (language) => !exists[language],
        )
        if (!size(unAcceptedLanguages)) {
          return null
        }
...
async function fetchACSolutions(question, languages) {
  const lastSubmissions = await Promise.all(
    map(languages, (language) => fetchLastSubmission(question, language)),
  )
  const filtered = filter(lastSubmissions, Boolean)
  ...
  return filter(solutions, Boolean)
}
